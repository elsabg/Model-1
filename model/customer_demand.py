...
def calc_elastic_mondemand(self, elec_price, d):
    ...
    return (self.hist_demand[d] *
            (1 - self.elasticity + self.elasticity * (elec_price / self.hist_price[d])))

def calc_elastic_monthdem_const(self, bin_price_curve, y, d):
    """calculates the monthly demand depending on electricity price"""
    return (self.hist_demand[d] *
            (1 - self.elasticity + self.elasticity * (quicksum(bin_price_curve[i, y] * self.price_steps for i in range(self.p_steps))
                                                      / self.hist_price[d])))
...
def disp_sum_year(self, year, disp):
    ...
    disp_year = quicksum(
        quicksum(disp[g, year, d, h] for g in self.techs_g) * self.d_weights[d]
...
def calc_disp_steps(self):
    ...
    demand_0 = sum(calc_elastic_mondemand(self, 0, d) * self.d_weights[d] for d in range(self.days))
    delta_q0 = demand_0 / self.p_steps
    price_0 = self.elasticity * (- demand_0)
    delta_p0 = price_0 / self.p_steps
    disp_steps = np.zeros(self.p_steps)
    price_steps = np.zeros(self.p_steps)
    for i in range(self.p_steps):
        disp_steps[i] = delta_q0 * (i + 1)
        price_steps[i] = delta_p0 * (i + 1)
    return disp_steps, price_steps
...
def mc_price_curve(self, year, disp):
    ...
    price = (disp_sum_year(self, year, disp)
             / calc_res_demand_year(self) + self.elasticity - 1) * (self.hist_price[0] / self.elasticity)
    return price